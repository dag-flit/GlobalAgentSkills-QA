...
export class AzureDevOpsAdapter extends TrackerAdapter {
  ...
  // Crea una HU de "hallazgos"/regresión SIN relacionarla a nada, en el sprint en curso, con la
  // evidencia (resumen + capturas inline + reporte adjunto). El cuerpo vive en ./ado-findings.mjs
  // (guardrail de 400 líneas); este método delega pasándose a sí mismo para reusar client + helpers.
  async createFindingsWorkItem(opts = {}) {
    return createFindingsWorkItem(this, opts);
  }

  // Sube UNA imagen y devuelve su URL de adjunto (para referenciarla INLINE en un campo HTML). Best-effort.
  async _uploadInline(filePath) {
    try {
      if (!filePath || !fs.existsSync(filePath)) return null;
      const up = await this.client.uploadAttachment(path.basename(filePath), fs.readFileSync(filePath));
      return (up.json && up.json.url) || null;
    } catch {
      return null;
    }
  }

  // Descubre el reference name del campo custom «Evidences» (o «Evidencias») por su nombre visible,
  // sin quemarlo. Prioriza el override del perfil (azure.fields.evidences). Cachea el resultado
  // (incluido null → «no existe») para no re-consultar. Best-effort.
  async _resolveEvidenceField() {
    if (this._evidenceFieldRef !== undefined) return this._evidenceFieldRef;
    const override = this._field("evidences", null);
    if (override) {
      this._evidenceFieldRef = override;
      return override;
    }
    try {
      const res = await this.client.listFields();
      const fields = (res.json && res.json.value) || [];
      const hit = fields.find(
        (f) => /^\s*evidenc/i.test(String(f.name || "")) || /\.evidenc/i.test(String(f.referenceName || ""))
      );
      this._evidenceFieldRef = hit ? hit.referenceName : null;
    } catch {
      this._evidenceFieldRef = null;
    }
    return this._evidenceFieldRef;
  }
