// failureExplain — espejo en webapp de runtime/evidence/failure-explain.mjs. Traduce el fallo
// TÉCNICO de una prueba a lenguaje LLANO (qué pasó / qué hacer) para no técnicos. Determinista,
// sin IA. Si no reconoce el patrón devuelve null → la UX muestra el mensaje crudo tal cual.
// Mantener los patrones en sync con el motor (el reporte md/html usa la versión .mjs).
...
export function explainFailure(
  ...
  const m = hay.match(
    /(?:Failed to resolve import|Cannot find module|Cannot find package|Module not found)[^"'`]*["'`]([^"'`]+)["'`]/i,
  );
  if (m) {
    const dep = m[1];
    const local = /^[./\\]/.test(dep);
    return {
  ...
  // 1b) Fallo por BASE DE DATOS / conexión no disponible (típico en pruebas de integración).
  ...
  // 2) Suite/archivo de prueba que no cargó por otra razón.
  ...
  // 3) Elemento de la interfaz que la prueba no encontró.
  ...
  // 4) Aserción: esperaba un valor y obtuvo otro.
  ...
  // 6) Seguridad (bandit/semgrep): riesgos, no "tests" rotos.
  if (layer === "security") {
...
// Explicación a NIVEL DE CAPA para un resultado con fallo (sobre todo caseless: dotnet-test/tsc/
// redocly salen ≠0 sin desglose por prueba). Intenta un patrón en la narrativa; si no, texto genérico.
export function explainLayerFailure(r: {
  layer?: string;
  status?: string;
  narrative?: string | null;
  metrics?: { tool?: string } | null;
} = {}): FailureExplanation | null {
  if (r.status !== "fail") return null;
  const tool = r.metrics?.tool || r.layer || "la herramienta";
  const ex = explainFailure({ name: r.layer, message: r.narrative || "" }, { layer: r.layer, tool: r.metrics?.tool });
  if (ex) return ex;
