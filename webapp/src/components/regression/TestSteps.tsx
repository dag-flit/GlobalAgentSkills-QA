...
function describeStep(s: RegressionStep, aliases: AliasOption[]): string {
  ...
  const val = friendlyValue(s.valor ?? s.texto ?? s.ruta ?? s.nombre ?? "");
  ...
  return (
    <div className="space-y-2">
      <ol className="space-y-1">
        ...
        {steps.map((s, i) => (
          <li key={i} className="flex items-center gap-2 text-[12px] bg-panel2/40 rounded px-2 py-1">
      ...
      <div className="flex items-end gap-2 flex-wrap border-t border-border pt-2">
        <label className="text-[11px] text-muted">
          Paso
          <select className="input block mt-0.5" value={op} onChange={(e) => { setOp(e.target.value); setAlias(""); setValue(""); }}>
            {STEP_TYPES.map((st) => (
        ...
        {t?.field && (
          <label className="text-[11px] text-muted">
            {t.field === "ruta" ? "Ruta" : t.field === "texto" ? "Texto" : t.field === "nombre" ? "Nombre" : "Valor"}
            <div className="mt-0.5 flex items-center gap-1">
              <input className="input" placeholder={t.placeholder} value={value} onChange={(e) => setValue(e.target.value)} />
              {t.field === "valor" && secrets && (
                <>
                  <button type="button" className="btn-ghost text-[11px] whitespace-nowrap" title="Usa el usuario guardado del sistema (cifrado; no se muestra ni se guarda el valor real)" onClick={() => setValue("${QA_USER}")}>🔐 Usuario</button>
                  <button type="button" className="btn-ghost text-[11px] whitespace-nowrap" title="Usa la clave guardada del sistema (cifrada; no se muestra ni se guarda el valor real)" onClick={() => setValue("${QA_PASS}")}>🔐 Clave</button>
                </>
              )}
            </div>
          </label>
        ...
        <button className="btn-ghost" onClick={addStep} disabled={!!t?.needsElement && !alias}>+ Agregar paso</button>
      </div>
