...
// Repositorio de runs. Upsert ATÓMICO (una sola sentencia INSERT…ON CONFLICT): dos
// escrituras concurrentes del mismo run serializan en el lock de fila → nunca queda
// una fila a medias. `version` se incrementa en cada update (lock optimista / auditoría).
...
export async function getRun(id: string): Promise<RunRecord | undefined> {
  const res = await query("SELECT * FROM runs WHERE id = $1", [id]);
  return res.rows[0] ? rowToRun(res.rows[0]) : undefined;
}

export async function listRuns(): Promise<RunRecord[]> {
  const res = await query("SELECT * FROM runs ORDER BY created_at DESC");
...
export async function markStopRequested(id: string): Promise<void> {
  await query("UPDATE runs SET stop_requested = true WHERE id = $1", [id]);
}
